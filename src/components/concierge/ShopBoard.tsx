...
import { isPastTime } from './constants';
...
export const ShopBoard: React.FC<ShopBoardProps> = ({ barbers, onBookSlot, selectedDate, onDateChange }) => {
  ...
  // Create a mounted state to prevent hydration errors and date mismatches
  const [mounted, setMounted] = useState(false);
  const [todayEST, setTodayEST] = useState('');

  useEffect(() => {
    setMounted(true);
    // Execute strictly on client to avoid Node vs Browser timezone discrepancies
    const estDateFormatter = new Intl.DateTimeFormat('en-CA', { timeZone: 'America/New_York' });
    setTodayEST(estDateFormatter.format(new Date()));
  }, []);

  const currentDate = selectedDate || todayEST;
  const isToday = todayEST !== '' && currentDate === todayEST;

  // Calculate if the shop is closed based on selected date
    ...
    <div className="bg-accent-dark/50 backdrop-blur border border-slate-700 rounded-xl overflow-hidden shadow-xl text-slate-300 h-full flex flex-col relative">
      ...
      <div className="flex border-b border-slate-700">
        <button
          ...
          className={`flex-1 py-3 text-sm font-bold uppercase tracking-wider transition-colors ${activeTab === 'schedule' ? 'bg-slate-700/50 text-white border-b-2 border-red-500' : 'hover:bg-slate-700/30 text-slate-500'
            }`}
        >
          The Board
        </button>
        <button
          ...
          className={`flex-1 py-3 text-sm font-bold uppercase tracking-wider transition-colors ${activeTab === 'prices' ? 'bg-slate-700/50 text-white border-b-2 border-red-500' : 'hover:bg-slate-700/30 text-slate-500'
            }`}
        >
          Price List
        </button>
      ...
      {/* Persistent Schedule Header */}
      {activeTab === 'schedule' && (
        <div className="flex-none flex flex-col border-b border-slate-700/50 p-4 bg-slate-900/50">
          <div className="flex items-center justify-between">
            <h3 className="text-lg font-bold text-white">
              {mounted && isToday ? "Today's Roster" : "The Roster"}
            </h3>
            <div className="flex items-center gap-3">
              {mounted && isToday && <span className="text-xs text-red-400 animate-pulse font-mono min-w-[50px]">● LIVE</span>}
              {mounted && (
                <input
                  type="date"
                  value={currentDate}
                  ...
                  className="bg-slate-800 text-slate-300 text-xs font-mono border border-slate-600 rounded px-2 py-1 outline-none focus:border-red-500 cursor-pointer"
                />
              )}
            </div>
          </div>
          <p className={`text-xs font-mono mt-1 ${isClosed ? 'text-red-500' : 'text-green-400'}`}>
            {mounted ? (
              isClosed ? 'Shop is CLOSED TODAY' : `Open ${isToday ? 'Today' : 'Date'}: 9:00 AM - 5:00 PM`
            ) : ''}
          </p>
        </div>
      )}
                      ...
                      {barber.isWorking ? (
                        <div className="grid grid-cols-4 gap-2">
                          {barber.schedule.map((slot, idx) => {
                            const past = isPastTime(slot.time, currentDate);
                            const isActuallyBooked = slot.isBooked || past;
                            return (
                              <button
                                key={idx}
                                disabled={isActuallyBooked}
                                onClick={() => handleSlotClick(barber, slot.time, isActuallyBooked)}
                                className={`
